...
*Goal: Substack-styl bloging with automated "Extreme Content" filtering.*

- [ ] **T3.1:** Design **blog Schema**: Define `title`, `content`, `author` (User reference), and `isFlagged`.
- [ ] **T3.2:** Build **Content Guard Middleware**: Regex-based scanner for "Extreme Content" (e.g., violence, murder).
- [ ] **T3.3:** Implement **blog Creation Logic**: Integrate `Content Guard` to automatically flag blogs.
- [ ] **T3.4:** Implement **Read Operations**: Fetch all blogs with `.populate('author')` for the Home feed.
- [ ] **T3.5:** Implement **Update/Delete Logic**: Secure routes so only the Author or Admin can modify content.
- [ ] **T3.6:** Create `/api/blogs` endpoints for all CRUD operations.
...
- [ ] **T4.1:** Design **Comment Schema**: Link to both `blog` and `User` models.
...
- [ ] **T4.4:** Create **Admin Dashboard Endpoint**: Fetch all blog posts marked as `isFlagged: true`.
- [ ] **T4.5:** Implement **Leaderboard Query**: Aggregate user points based on blog post frequency and engagement.
...
- [ ] **T5.4:** Final Deployment: Deploy backend to **x** and connect the MongoDB Atlas cluster.
